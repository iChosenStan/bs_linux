...
std::vector<ProcessInfo> StealthInjection::FindProcesses(const std::string& name) {
    ...
    while ((entry = readdir(procDir)) != nullptr) {
        ...
        // Read process name
        std::string comm = ReadFileString("/proc/" + std::string(entry->d_name) + "/comm");
        std::string cmdline = ReadFileString("/proc/" + std::string(entry->d_name) + "/cmdline");
        ...
        if (name.empty()) {
            ...
            else if (comm.find(name) != std::string::npos && comm.find(".exe") != std::string::npos) {
                matches = true;
            }
            // Priority 3: Check cmdline but ignore the injector's own folder (matches IDE tools)
            else if (cmdline.find(name) != std::string::npos && cmdline.find("Documents/GitHub") == std::string::npos) {
                matches = true;
            }
        }
        ...
        if (info.isWine || 
            info.path.find(".exe") != std::string::npos ||
    ...
    // If we found sandbox processes but no direct matches, look for child processes
        ...
        for (const auto& sandbox : sandboxProcesses) {
            ...
            if (procDir2) {
                ...
                while ((entry2 = readdir(procDir2)) != nullptr) {
                    ...
                    std::string statPath = "/proc/" + std::string(entry2->d_name) + "/stat";
                    ...
                    if (statFile.is_open()) {
                        ...
                        if (ppid == (int)sandbox.pid || ppid == 1) {
                            ...
                            ProcessInfo info = {};
                            info.pid = childPid;
                            info.name = ReadFileString("/proc/" + std::string(entry2->d_name) + "/comm");
                            info.isWine = IsWineProcess(childPid);
                            
                            char exeBuf[512];
                            std::string exePath = "/proc/" + std::string(entry2->d_name) + "/exe";
                            ssize_t len = readlink(exePath.c_str(), exeBuf, sizeof(exeBuf) - 1);
                            if (len > 0) {
                                exeBuf[len] = '\0';
                                info.path = exeBuf;
                            ...
                            // Check if this matches our target
                            std::string childCmdline = ReadFileString("/proc/" + std::string(entry2->d_name) + "/cmdline");
                            if (info.name.find(name) != std::string::npos ||
                                info.path.find(name) != std::string::npos ||
                                childCmdline.find(name) != std::string::npos ||
                                info.isWine) {
                                results.push_back(info);
                            }
                        }
...
std::optional<ProcessInfo> StealthInjection::FindProcess(const std::string& name) {
    auto processes = FindProcesses(name);
    if (processes.empty()) return std::nullopt;
    return processes[0];
}
...
std::optional<ProcessInfo> StealthInjection::FindProcessByPid(uint32_t pid) {
    ...
    if (comm.empty()) return std::nullopt;
...
std::optional<ProcessInfo> StealthInjection::FindWineProcess(const std::string& winePrefix, const std::string& processName) {
    auto processes = FindProcesses(processName);
    for (const auto& proc : processes) {
        if (proc.isWine && (winePrefix.empty() || proc.winePrefix.find(winePrefix) != std::string::npos)) {
            return proc;
        }
    }
    return std::nullopt;
...
std::optional<ModuleInfo> StealthInjection::GetModule(uint32_t pid, const std::string& name) {
...
uintptr_t StealthInjection::GetProcAddress(uint32_t pid, uintptr_t moduleBase, const std::string& symbol) {
    // Robust ELF symbol resolution
    Elf64_Ehdr ehdr;
    if (!ReadProcessMemory(pid, moduleBase, &ehdr, sizeof(ehdr))) return 0;
    ...
    for (int i = 0; i < ehdr.e_phnum; i++) {
        if (!ReadProcessMemory(pid, moduleBase + ehdr.e_phoff + (i * ehdr.e_phentsize), &phdr, sizeof(phdr))) continue;
    ...
    if (dynamicAddr == 0) return 0;
    
    // Some libraries have absolute virtual addresses, some have relative
    if (dynamicAddr < moduleBase) dynamicAddr += moduleBase;
    ...
    while (true) {
        if (!ReadProcessMemory(pid, currentDyn, &dyn, sizeof(dyn))) break;
    ...
    for (int i = 0; i < 5000; i++) {
        if (!ReadProcessMemory(pid, symtab + (i * sizeof(sym)), &sym, sizeof(sym))) break;
        if (sym.st_name == 0 && i > 0) continue; // Skip empty but not first
        
        if (!ReadProcessMemory(pid, strtab + sym.st_name, nameBuf, sizeof(nameBuf))) continue;
...
bool StealthInjection::ReadProcessMemory(uint32_t pid, uintptr_t address, void* buffer, size_t size) {
    struct iovec local = {
        .iov_base = buffer,
        .iov_len = size
    };
...
bool StealthInjection::WriteProcessMemory(uint32_t pid, uintptr_t address, const void* buffer, size_t size) {
    struct iovec local = {
...
std::vector<uint32_t> StealthInjection::GetThreads(uint32_t pid) {
    ...
    while ((entry = readdir(taskDir)) != nullptr) {
        // Skip '.' and '..' fast; also skip non-dir entries on DT_UNKNOWN filesystems
        if (entry->d_name[0] == '.') continue;
        if (entry->d_type != DT_DIR && entry->d_type != DT_UNKNOWN) continue;
        try {
            uint32_t tid = std::stoul(entry->d_name);
            if (tid > 0) threads.push_back(tid);
        } catch (...) {
            continue;
        }
    }
...
// The thread must already be stopped via PTRACE_ATTACH + waitpid before calling.
static uintptr_t AllocateRemoteMemoryInProcess(uint32_t pid, uint32_t tid,
    const struct user_regs_struct& origRegs, size_t size, int protection) {
    ...
    long trapWord = (origWord & (long)0xFFFFFF0000000000LL) | (long)0x00000000CC050FLL;
...
uintptr_t StealthInjection::AllocateRemoteMemory(uint32_t pid, size_t size, int protection) {
    ...
    if (ptrace(PTRACE_ATTACH, tid, nullptr, nullptr) == -1) return 0;
    ...
    uintptr_t result = AllocateRemoteMemoryInProcess(pid, tid, origRegs, size, protection);

    ptrace(PTRACE_DETACH, tid, nullptr, nullptr);
    return result;
}

bool StealthInjection::CallRemoteFunction(uint32_t pid, uintptr_t function, uintptr_t* args, size_t argCount, uintptr_t* result) {
...
bool StealthInjection::CheckAntiCheat(uint32_t pid) {
    ...
    for (const auto& mod : modules) {
        ...
        if (name.find("easyanticheat") != std::string::npos ||
            name == "eac.dll" || name == "eac.so" ||
            name.find("battleye") != std::string::npos ||
...
std::vector<uint8_t> StealthInjection::ReadLibraryFile(const std::string& path) {
...
bool StealthInjection::InjectPtrace(uint32_t pid, const std::string& libraryPath, InjectionResult& result) {
    ...
    for (uint32_t t : threads) {
        std::string stPath = "/proc/" + std::to_string(pid)
                           + "/task/" + std::to_string(t) + "/status";
    ...
    if (!tid) tid = threads[0];  // Fallback to first thread
    printf("[+] Selected thread %u for injection\n", tid);

    // === Attach once — stay attached for all operations ===
    if (ptrace(PTRACE_ATTACH, tid, nullptr, nullptr) == -1) {
        result.error = std::string("Failed to attach to thread ") + std::to_string(tid)
                       + " (" + strerror(errno) + ")";
        return false;
    }
    ...
    if (!dlopenAddr) dlopenAddr = GetProcAddress(pid, libcMod.base, "__libc_dlopen_mode");
    if (!dlopenAddr) {
        for (const auto& mod : modules) {
            std::string ln = mod.name, lp = mod.path;
            std::transform(ln.begin(), ln.end(), ln.begin(), ::tolower);
    ...
    // This fixes SIGSEGV crashes by providing a clean, isolated stack for dlopen
    // that doesn't interfere with the target thread's original stack.
    //
    ...
    //
    // RSP will point to the TOP of the stack area (0x2000) for proper growth downward.
    ...
    constexpr size_t kStackOffset = 512;  // Stack starts here and grows downward from top
    ...
    if (pathSize > kPathSize) {
        result.error = "Library path too long (max " + std::to_string(kPathSize - 1) + " chars)";
        ptrace(PTRACE_DETACH, tid, nullptr, nullptr);
        return false;
    }
    ...
    // === Build shellcode: call dlopen(pathMem, RTLD_NOW=2); store result; int3 ===
    // We use a proper call/return pattern with a valid return address within our shellcode.
    //
    // Shellcode layout:
    //   [0x00-0x1A] Setup: mov rax,dlopen; mov rdi,path; mov esi,2; call rax
    //   [0x1B]      int3 trap
    //
    // Stack layout before execution:
    //   [RSP] = address of int3 (return address for call rax)
    //
    // This ensures dlopen returns to int3 within our shellcode.
    
    // Calculate the address of int3 (shellcode + offset of int3 instruction)
    const size_t kInt3Offset = 27;  // int3 is at byte 27
    uintptr_t int3Addr = shellcodeMem + kInt3Offset;
    
    uint8_t shellcode[] = {
        0x48, 0xB8, 0,0,0,0,0,0,0,0,   // 0x00: movabs rax, dlopenAddr
        0x48, 0xBF, 0,0,0,0,0,0,0,0,   // 0x0A: movabs rdi, pathMem  
        0xBE, 0x02, 0x00, 0x00, 0x00,   // 0x14: mov esi, RTLD_NOW
        0xFF, 0xD0,                      // 0x19: call rax
        0xCC                             // 0x1B: int3
    };
    memcpy(shellcode + 2,  &dlopenAddr, 8);
    memcpy(shellcode + 12, &pathMem,    8);
    ...
    printf("[+] Shellcode at 0x%lX (%zu bytes), int3 at 0x%lX\n", 
           shellcodeMem, sizeof(shellcode), (unsigned long)int3Addr);
    ...
    //   3. Write the address of int3 as return address so dlopen returns to our trap.
    ...
    regs.rsp = (regs.rsp - 8) & ~(uintptr_t)0xFULL;  // Align to 16 bytes and reserve space
    ...
    // Zero caller-saved registers that dlopen doesn't expect to be arbitrary values
...
bool StealthInjection::InjectMemfd(uint32_t pid, const std::vector<uint8_t>& libraryData, InjectionResult& result) {
    printf("[*] Using memfd injection method\n");

    // Strategy: create an anonymous in-memory file (memfd) in the INJECTOR
    // process, write the .so there, then hand dlopen the path
    // /proc/<injector_pid>/fd/<N>. The target follows this symlink through
    // procfs — no file is ever written to disk.
    ...
    // 3. Build the path the TARGET will use to dlopen our memfd.
    //    /proc/<injector_pid>/fd/<N> is a symlink into our fd table.
    char memfdPath[128];
    snprintf(memfdPath, sizeof(memfdPath),
             "/proc/%d/fd/%d", static_cast<int>(getpid()), memfd);
    ...
    if (ok) printf("[+] Memfd injection successful (no disk footprint)\n");
    return ok;
}


bool StealthInjection::InjectPreload(const std::string& libraryPath, const std::string& targetCmd) {
...
bool StealthInjection::InjectWine(uint32_t pid, const std::string& libraryPath, InjectionResult& result) {
    ...
    if (!hasLibc) {
        ...
        printf("[!] The game may be running in a sandbox that prevents injection\n");
        printf("\n");
        ...
        result.warnings.push_back("For Wine/Proton, LD_PRELOAD before game start is recommended");
        return false;
    }
...
InjectionResult StealthInjection::Inject(const InjectionConfig& config) {
    ...
    if (!procInfo) {
        result.error = "Target process not found: " + 
            (config.targetPid ? std::to_string(config.targetPid) : config.targetProcess);
        return result;
    }
    ...
    if (CheckAntiCheat(procInfo->pid)) {
        result.warnings.push_back("Anti-cheat detected - injection may be detected");
    ...
    switch (method) {
        case 1: printf("Ptrace\n"); break;
        case 2: printf("Memfd\n"); break;
        case 3: printf("LD_PRELOAD\n"); break;
        case 4: printf("Wine/Proton\n"); break;
        default: printf("Auto\n"); break;
    }
